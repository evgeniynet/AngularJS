...
export class InvoiceDetailsPage {
    ...
    addRecipientsButton() {
        ...
        myModal.onDismiss(data1 => {
            ...
            {
                data1.fullname=data1.name;
                data1.email=data1.name;
            }
            data1.is_accounting_contact=true;
            this.invoice.recipients = this.invoice.recipients || [];
            if (!this.invoice.recipients.filter(v => v.email == data1.email).length)
            {
